...
row = row.drop('date', 'bedrooms', 'sqft_lot', 'condition', 'sqft_basement', 'yr_built', 'yr_renovated', 'zipcode',
               'lat', 'long', 'sqft_lot15')
...
XTX = X.rdd.map(lambda s: (1, np.outer(np.array(s).astype(np.float), np.array(s).astype(np.float)))).reduceByKey(
    lambda a, b: a + b).collect()[0][1]
inverse_XTX = np.linalg.inv(np.asarray(XTX))
XTy = X_with_y.rdd.map(
    lambda s: (1, np.outer(np.array(s).astype(np.float)[1:], np.array(s).astype(np.float)[0]))).reduceByKey(
    lambda a, b: a + b).collect()[0][1]
